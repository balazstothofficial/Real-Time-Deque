...
next
  ...
  proof(induction "Idle.push x left'")
    ...
    next
      ...
      from False have invar: "States.invar ?states"
        apply(auto)
...
next
  ...
  from 6 have invar: "States.invar ?newStates"
    by (meson invar.simps(6) States.invar.simps(1) invar_push_small)
...
next
  ...
  from 7 have invar: "States.invar ?newStates"
    by (meson RealTimeDeque.invar.simps States.invar.simps invar_push_big)
...
lemma remaining_steps_idle: "States.invar (States dir small big) \<Longrightarrow> remaining_steps (States dir small big) = 0 \<longleftrightarrow> (
...
next
  ...
  proof(induction "Idle.push x left'")
    ...
    next
     ...
     from invar have "States.invar (six_steps ?states)"
       using invar_n_steps by blast
...
next
  ...
  next
    ...
    then have "remaining_steps ?newStates \<le> 4"
      by (metis invar.simps(6) leI remaining_steps_push_small)
    ...
    have small_sizes_1: "Small.size ?newSmall = Suc (Small.size small)"
      by (meson "6.prems" invar.simps(6) States.invar.simps(1) funpow_0 step_n_push_size_small)
    ...
    then have "0 < Small.size_new small"
      using "6.prems" invar.simps(6) size_ok.simps(1) States.invar.simps(1) size_ok_size_new_small by blast

    then have "0 < Small.size_new ?newSmall"
      using \<open>Small.size (Small.push x small) = Suc (Small.size small)\<close> invar Small_Proof.size_size_new by fastforce
    ...
    then have "0 < Big.size_new big"
      using "6.prems" invar.simps(6) size_ok.simps(1) States.invar.simps(1) size_ok_size_new_big by blast
    ...
    have "Suc (Small.size_new small) = Small.size_new ?newSmall"
      by (metis "6.prems" invar.simps(6) funpow_0 step_n_push_size_new_small)
    ...
    then have "Big.size_new big \<le> 3 * Small.size_new ?newSmall"
      using small_sizes_1
      by (metis (no_types, opaque_lifting) "6.prems" invar.simps(6) dual_order.trans funpow_0 le_add2 mult_le_mono2 plus_1_eq_Suc step_n_push_size_new_small)
...
next
  ...
  from 7 have invar: "States.invar ?newStates"
    by (meson invar.simps(6) States.invar.simps invar_push_big)
  ...
  next
    case False
    then have "remaining_steps ?newStates \<le> 4"
      by (metis invar.simps(6) leI remaining_steps_push_big)
    ...
    have big_sizes_1: "Big.size ?newBig = Suc (Big.size big)"
    ...
    then have "0 < Big.size_new big"
     using "7.prems" invar.simps(6) size_ok.simps(1) size_ok_size_new_big
     by fastforce

    then have "0 < Big.size_new ?newBig"
      using \<open>Big.size (Big.push x big) = Suc (Big.size big)\<close> invar Big_Proof.size_size_new by fastforce

    then have "0 < Big.size_new steppedBig"
      by (simp add: \<open>Big.size_new (Big.push x big) = Big.size_new steppedBig\<close>)

    then have "0 < Small.size_new small"
      using "7.prems" invar.simps size_ok.simps size_ok_size_new_small
      by fastforce
    ...
    with idle states_invar T have "invar (Idle bigIdle smallIdle)"
      apply auto
      apply (metis Idle.is_empty.elims(2) Idle.size.simps Stack_Proof.size_empty Suc_neq_Zero \<open>Suc (Big.size_new big) = Big.size_new (Big.push x big)\<close> big_size big_sizes)
